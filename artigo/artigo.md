...
O Windows Subsystem for Linux 2 (WSL2) tornou-se uma plataforma popular para execução de ferramentas de benchmarking em computadores pessoais com Windows, especialmente em contextos acadêmicos onde o acesso a hardware bare-metal é limitado. No entanto, o WSL2 utiliza o hipervisor Hyper-V da Microsoft, que introduz uma camada de virtualização entre o sistema operacional Linux convidado e o hardware físico. Este trabalho avalia quantitativamente o impacto dessa virtualização no desempenho de três subsistemas: armazenamento (I/O aleatório em SSD NVMe PCIe 4.0), largura de banda de memória e processamento paralelo de CPU. Os experimentos foram conduzidos em um processador Intel Core i5-1235U (6 núcleos físicos / 12 threads lógicos) com SSD SKHynix HFS512GEJ4K112N NVMe PCIe 4.0 x4, executando Ubuntu 24.04 LTS via WSL2 no Windows 11. Cada configuração experimental foi repetida 30 vezes, com 3 execuções de aquecimento descartadas, e os resultados foram analisados com intervalo de confiança de 95 % pela distribuição t de Student. No subsistema de armazenamento, o WSL2 atingiu 191.733 IOPS (IC 95 %: [181.901; 201.565]) em leitura aleatória 4K com fila de profundidade 64, apresentando coeficiente de variação de 13,7 %, sinal de instabilidade não-determinística na pilha VirtIO-blk/Hyper-V. A largura de banda de memória medida pelo mbw (3.536 a 4.172 MiB/s) ficou muito abaixo do limite teórico do DDR4-1600 em modo dual-channel (aprox. 25,6 GB/s), com padrão anômalo de variação por tamanho de array e agravado pela limitação de 3,7 GiB de RAM visíveis dentro do WSL2. O desempenho de CPU escala razoavelmente até 6 threads com eficiência acima de 85 %, caindo para 66,0 % em 12 threads, comportamento explicado pela Lei de Amdahl, overhead de sincronização e contenção de cache L3. Conclui-se que o WSL2 é inadequado como ambiente de benchmarking de armazenamento e memória, mas oferece desempenho de CPU representativo para cargas CPU-bound.
...
O Windows Subsystem for Linux 2 (WSL2), lançado pela Microsoft em 2020, tornou-se uma das formas mais convenientes de executar distribuições Linux completas em computadores com Windows 10 ou 11. Diferente de seu predecessor (WSL1), que traduzia chamadas de sistema Linux para chamadas equivalentes do Windows NT, o WSL2 executa um kernel Linux real, mantido pela própria Microsoft, sobre o hipervisor Hyper-V, o mesmo utilizado em ambientes de virtualização corporativa e na infraestrutura de nuvem Azure (MICROSOFT, 2019).

Essa mudança arquitetural conferiu ao WSL2 compatibilidade binária quase total com ambientes Linux nativos: ferramentas como Docker, compiladores, shells e utilitários de sistema funcionam sem modificações. Em disciplinas de infraestrutura de hardware e sistemas operacionais, o WSL2 tem sido amplamente adotado como plataforma de laboratório por eliminar a necessidade de dual-boot ou hardware dedicado, opções nem sempre disponíveis para estudantes.

A camada de virtualização imposta pelo Hyper-V, contudo, tem consequências diretas para o desempenho do sistema. O acesso ao armazenamento não ocorre diretamente via driver NVMe do host, mas por meio de uma pilha paravirtualizada (VirtIO-blk sobre VMBus) que envolve múltiplas transições entre os domínios do convidado e do anfitrião. De forma análoga, a memória física é gerenciada pelo Hyper-V em páginas que podem ser realocadas dinamicamente, e o escalonamento de threads de CPU compete com outros processos do Windows host pelo mesmo conjunto de núcleos físicos.
...
**H0 (nula):** O WSL2 não introduz degradação estatisticamente significativa no desempenho dos subsistemas avaliados em relação às especificações nominais do hardware em ambiente bare-metal. Os intervalos de confiança de 95 % das medições incluiriam os valores de referência publicados para Linux nativo no mesmo hardware.

**H1 (alternativa):** O WSL2 introduz degradação mensurável e estatisticamente significativa em pelo menos um dos três subsistemas avaliados, com os valores medidos situando-se sistematicamente abaixo dos intervalos esperados para ambiente bare-metal.

### 1.3 Justificativa

Se os resultados obtidos no WSL2 divergem significativamente do desempenho bare-metal, conclusões sobre o comportamento do hardware baseadas nessas medições podem ser incorretas. A questão tem relevância prática direta para pesquisadores, estudantes e profissionais que utilizam o WSL2 como ambiente de benchmarking. Este trabalho fornece dados empíricos quantitativos para subsidiar essa decisão, empregando n = 30 repetições por configuração com análise estatística rigorosa.
...
O restante deste artigo está organizado da seguinte forma: a Seção 2 apresenta os trabalhos relacionados; a Seção 3 descreve a metodologia experimental; a Seção 4 apresenta os resultados; a Seção 5 discute as implicações; e a Seção 6 traz as conclusões e trabalhos futuros.
...
O SKHynix HFS512GEJ4K112N é um SSD NVMe M.2 2280 com interface PCIe 4.0 ×4 e capacidade de 512 GB. A interface PCIe 4.0 ×4 oferece largura de banda teórica máxima de aprox. 7,88 GB/s (4 lanes × 2 GB/s/lane × 2 direções, porém unidirecional aprox. 3,94 GB/s por sentido). No ambiente WSL2, a leitura sequencial medida com fio e `direct=1` atingiu 4.112 MB/s (aprox. 52 % do limite teórico unidirecional da interface), enquanto a leitura aleatória 4K de referência é de 62.300 IOPS. Esses valores constituem o referencial para comparação dos resultados obtidos no experimento de 30 repetições com iodepth=64 (SK HYNIX, 2022).
...
| Largura de Banda Teórica | aprox. 25,6 GB/s (DDR4-1600 dual channel) |
...
O ambiente de software é descrito na Tabela 2. O WSL2 executa sobre o Hyper-V do Windows 11, que atua como hipervisor de tipo 1 nessa configuração. No Windows 11 com Hyper-V habilitado, o próprio Windows corre em uma partição privilegiada do Hyper-V (root partition), enquanto o WSL2 ocupa uma partição filha (child partition). Essa arquitetura implica que o acesso ao hardware NVMe pelo WSL2 percorre duas fronteiras de virtualização.
...
**Figura 1.** IOPS de leitura aleatória 4K medidos com fio no WSL2 (n=30, iodepth=64, direct I/O). A barra de erro representa o IC 95 %. O coeficiente de variação elevado (CV = 13,7 %) evidencia instabilidade na pilha de I/O virtualizada.

**Tabela 3: Resultados Estatísticos de I/O Aleatório (fio, n = 30)**
...
Os CVs de 13,7 % para IOPS e 17,3 % para latência são muito superiores ao CV típico observado em benchmarks de NVMe em Linux nativo (geralmente abaixo de 3–5 % com as mesmas configurações). A análise dos dados brutos em `dados/raw/fio_aleatorio.csv` mostra amplitude expressiva: o mínimo de 120.000 IOPS (run 3) e o máximo de 227.000 IOPS (run 25) representam uma variação de 89 % entre o pior e o melhor caso, amplitude inaceitável para caracterização de hardware.
...
**Tabela 4: Resultados Estatísticos de Largura de Banda de Memória (mbw, n = 30)**
...
Entre os três tamanhos testados, o array de 128 MiB apresentou vazão de 4.171,6 MiB/s, valor 18 % acima do array de 16 MiB (3.536,2 MiB/s). Em hardware bare-metal, o padrão esperado é o inverso: arrays menores cabem em caches de CPU e são acessados mais rapidamente (comportamento verificado em benchmarks de memória em Linux nativo com STREAM ou mbw, onde 16 MiB tipicamente produz maior throughput que 1.024 MiB). A variabilidade cresce com o tamanho do array: CV de 8,8 % (16 MiB), 13,0 % (128 MiB) e 23,8 % (1.024 MiB), sugerindo instabilidade crescente na gestão de grandes alocações de memória virtual.
...
**Figura 3.** Speedup de processamento (eventos/s relativo a 1 thread) versus número de threads no sysbench prime. A linha tracejada representa o speedup ideal linear. As barras de erro representam o IC 95 % do speedup. A divergência cresce a partir de 6 threads, reflexo da Lei de Amdahl, overhead de sincronização e contenção de cache L3.
...
**Tabela 5: Resultados Estatísticos de Paralelismo de CPU (sysbench, n = 30 por configuração)**
...
O CV do experimento de CPU situa-se entre 4,2 % e 8,2 %, consistentemente menor que os dos experimentos de armazenamento (13,7–17,3 %) e de memória (8,8–23,8 %), indicando que a virtualização de CPU no Hyper-V é significativamente mais estável e eficiente do que a virtualização de I/O e memória. O pico de CV em 4 threads (8,2 %) reflete maior variabilidade no escalonamento quando múltiplos núcleos físicos passam a ser utilizados simultaneamente, com competição crescente por cache L3 compartilhado.
...
Entre os três subsistemas avaliados, o de armazenamento foi o que apresentou maior variabilidade. A título de referência, a leitura sequencial medida nesta máquina com fio e `direct=1` atingiu 4.112 MB/s, e a leitura aleatória 4K de referência é de 62.300 IOPS. No experimento de 30 repetições com iodepth=64, o WSL2 atingiu 191.733 IOPS em média, valor superior ao da referência de fila simples porque o iodepth=64 explora o paralelismo interno do controlador NVMe, emitindo 64 requisições simultâneas. Ainda assim, o coeficiente de variação de 13,7 % evidencia instabilidade severa que não ocorre em condições nativas.

A causa fundamental reside na pilha de I/O do WSL2, que envolve as seguintes camadas entre o processo de benchmarking e o hardware:
...
Cada transição entre as camadas 2 e 3 e entre 3 e 4 envolve notificações inter-VM (doorbell writes e IPI, Inter-Processor Interrupts) com latências da ordem de microssegundos. Com profundidade de fila de 64 operações pendentes, essas latências se acumulam, elevando a latência média para 337,51 µs, com desvio padrão de 58,44 µs (CV = 17,3 %).

A variabilidade elevada (13,7 % para IOPS, 17,3 % para latência) reflete a natureza não-determinística do scheduler do VMBus e a competição por recursos de I/O entre o WSL2 e outros processos Windows em execução no host. A amplitude entre o mínimo (120.000 IOPS) e o máximo (227.000 IOPS), variação de 89 % entre o pior e o melhor caso, torna os resultados não confiáveis para caracterização de hardware: benchmarks de armazenamento no WSL2 não são representativos do desempenho real do hardware, invalidando H0 para este subsistema.
...
Os valores de largura de banda de memória (3.536 a 4.172 MiB/s, aprox. 3,5 a 4,1 GB/s) estão significativamente abaixo do limite teórico do DDR4-1600 em modo dual-channel (aprox. 25,6 GB/s = 24.414 MiB/s), o que representa apenas 14–17 % da capacidade teórica. Para um benchmark de cópia de memória em Linux nativo, o esperado seria atingir 30–50 % da largura de banda teórica, ou seja, 7.300 a 12.200 MiB/s. Os valores medidos ficam abaixo até desse intervalo conservador, sugerindo interferência da camada de virtualização.
...
O comportamento de CPU foi o menos afetado pela virtualização, resultado consistente com a literatura (KIVITY et al., 2007). O CV baixo e sistemático (4–8 %) confirma que o Hyper-V entrega computação essencialmente bare-metal para cargas CPU-bound, pois as instruções de aplicação são executadas diretamente no hardware via Intel VT-x sem tradução.

O speedup passa de 1,00× (1 thread) a 7,92× (12 threads), com eficiência caindo de 100 % para 66 %, e essa sub-linearidade é explicada por três mecanismos complementares:
...
O valor previsto (7,74×) é muito próximo do observado (7,92×), indicando que a fração serial do sysbench é o principal fator limitante ao escalonamento máximo. Para 6 threads, a mesma lei prevê S(6) = 1/(0,05 + 0,95/6) ≈ 5,14×, também consistente com o resultado experimental de 5,10×.
...
A combinação dos três fatores (fração serial por Amdahl, overhead de sincronização e contenção de L3) explica coerentemente a curva de eficiência: aproximadamente constante entre 1 e 6 threads (100 % a 85 %), com queda abrupta ao dobrar para 12 threads (66 %).
...
**Para benchmarks de armazenamento:** O WSL2 não é adequado. O CV de 13,7 % e a amplitude de 89 % entre o pior e o melhor caso tornam os resultados não representativos do hardware. Alternativas incluem Linux nativo (dual-boot ou live USB), Docker em Linux, ou máquina virtual com passthrough NVMe (se suportado pelo hypervisor).
...
**Para benchmarks de CPU:** O WSL2 é adequado para cargas CPU-bound, desde que se compreenda que a sub-linearidade no escalonamento, prevista pela Lei de Amdahl e agravada por sincronização e contenção de cache L3, não é artefato da virtualização, mas comportamento esperado do hardware paralelo.
